...
---
🦋 PS：上述流传会比较重（纯本地流，体验的同学可以按照上述的流）
要求你在自己的电脑里安装一个 MySQL 软件。简单说所有数据（文章、作品）都锁在你的这台 Mac 里。免费。
一旦你想把网站发布到互联网上让别人看，你还得再找一个云数据库把数据搬过去。所以可以按照下面的步骤：
---

## 进阶玩法：现代云原生部署流 (强烈推荐)

> **适合谁**：想要最快把网站发布到互联网、不想在本地电脑安装沉重数据库软件的现代开发者与设计师。
> 
> **核心逻辑**：本地电脑仅作代码编辑，数据库托管于 Aiven（云端），最终一键部署至 Vercel。全程极简、免费，且数据永不丢失。

### Step 1: 注册 Aiven 云数据库 (云端存储，永不丢失)

传统的本地安装数据库不仅极易报错，且换台电脑就无法工作。本项目推荐直接使用 **Aiven** 提供的免费企业级云数据库，永远在线，配置极简。

1. **注册账号**：访问 [Aiven 官网](https://console.aiven.io/)，使用你的邮箱或 GitHub 一键注册登录。
2. **创建服务**：点击页面上的 **Create service**，选择 **MySQL**。套餐 (Plan) 务必选择 **Free Plan**（免费版对个人作品集来说完全足够）。
3. **获取“钥匙”**：等待几十秒，当数据库状态变成绿色的 **Running** 时，在概览 (Overview) 页面找到 **Connection information (连接信息)**。复制那串超长的 **Service URI**，这就是你的专属数据库链接。
4. **【极度重要】解开门禁**：Aiven 默认有极高的安全防火墙。为了让你电脑里的代码能顺利连上它，点击左侧菜单的 **Service settings**，找到 **Allowed IP Addresses (允许的 IP 地址)**。点击添加并输入 `0.0.0.0/0`（代表允许所有网络访问），然后点击保存。

### Step 2: 本地环境与代码配置

1. 确保电脑已安装 [Node.js](https://nodejs.org/)。
2. 将项目克隆或下载到本地，使用 **Cursor**（或 VS Code 等 AI 编程工具）打开该文件夹。
3. 在项目根目录，找到 `.env.example` 文件，将其复制一份并重命名为 `.env`。
4. 打开 `.env` 文件，将第一行 `DATABASE_URL` 的值替换为你在 Step 1 复制的 Aiven Service URI 长链接。

### Step 3: 打通数据并本地点火

在 Cursor 底部的终端（Terminal）中，依次敲入以下命令并回车：

```bash
# 1. 安装项目必需的依赖零件
npm install

# 2. 将本地的数据库结构推送到 Aiven 云端
# (⚠️ 避坑指南：如果这里报错 P1001，通常是被你的科学上网 VPN 拦截了特殊端口。请将代理切换为“全局模式”或暂时关闭代理后再试)
npx prisma db push

# 3. 注入网站默认配置和初始管理员账号 (非常重要，解决网页白屏的关键！)
npm run db:seed

# 4. 启动本地预览服务器
npm run dev

成功后，在浏览器打开 `http://localhost:3000` 即可看到你的网站。你可以进入 `http://localhost:3000/admin`，使用默认账号（`admin@example.com` / `admin123`）登录后台进行测试和内容修改。

### Step 4: 使用 Vercel 免费部署上线 (向全世界展示)

当你在本地 `localhost:3000` 确认网站完美无缺后，就可以把它发布到互联网上，拥有属于你自己的专属链接。Vercel 是目前对前端开发者最友好的自动化部署平台。

1. **提交代码**：确保你已经将本地修改好的代码提交并 Push 到你自己的 GitHub 仓库。
2. **关联 Vercel**：访问 [Vercel 官网](https://vercel.com/)，选择使用 GitHub 账号一键登录。
3. **导入项目**：点击工作台右上角的 **Add New... -> Project**。在列表里找到你刚才创建的个人专属 GitHub 仓库（无需是公开仓库，Private 私有仓库同样支持），点击 **Import**。
4. **配置环境变量**：在部署前的设置界面，展开 **Environment Variables (环境变量)** 面板。你需要把本地 `.env` 文件里的关键信息填入这里：
   - **Name** 填入：`DATABASE_URL`
   - **Value** 填入：刚才 Aiven 里的那串包含密码的长链接
   - 点击 **Add** 添加。
5. **点火升空**：点击下方的 **Deploy** 按钮。Vercel 会自动拉取你的代码并进行打包。只需等待约 2-3 分钟，屏幕上开始撒花时，你的个人网站就正式诞生了！Vercel 会免费分配给你一个安全稳定的网址（后续你也可以在设置里绑定自己购买的独立域名）。
...
| 前台展示位置 | 默认值 | 后台修改路径 |
|---|---|---|
| 导航 Logo、浏览器标签页标题、面包屑首页 | Janie's Studio | 网站设置 → 基本设置 → 网站名称 |
...
|---|---|---|---|
| 页脚 `© 2026 XXX` 中的名称 | Janie | 网站设置 → 基本设置 → 版权文字 | 年份由系统自动生成 |
...
> Hero 第二行完整显示为：前缀 + 网站名称（斜体），例如 "You're in *Janie's Studio*"
...
| 个人名称标签 + 文章作者名 | Janie | 网站设置 → 关于我/头像 → 个人名称 |
...
| 选项 | 后台修改路径 |
|---|---|
| Janie（默认）/ 海蓝 / 翠绿 / 暖橙 / 正红 / 玫瑰 / 紫罗兰 / 琥珀 / 极简 | 网站设置 → 外观主题 → 强调色 |
...
```bash
git clone https://github.com/你的用户名/Janiestudio.git
```
...
3. 修改 .env 中的 DATABASE_URL，把密码部分改为：你的密码
   （例如 mysql://root:你的密码@localhost:3306/Ja niestudio）
4. 在 MySQL 中创建数据库，执行：CREATE DATABASE Janiestudio;
...
| `prisma migrate deploy` 报错 | `prisma migrate deploy 报错了，请帮我检查 DATABASE_URL 是否正确配置，以及 Janiestudio 数据库是否已创建` |
